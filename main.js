 ...
 new Vue({
  ...
  methods: {
   ...
   favoriteButtonClick(index, book) {
    var delete_number = 0;
    //既にお気に入り登録している場合は解除する
    if (this.featured_selectFavorite[index] === index &&book.category==="featured_book") {
     console.log("既にお気に入り登録されているので解除");
     for (let i = 0; i < this.favorite_books.length; i++) {
      if (this.favorite_books[i].isbn === book.isbn) {
       delete_number = i;
      }
     }
     this.favoriteButtonDelete(delete_number, this.favorite_books[delete_number]);
    }
    else if (this.selectFavorite[index] === index&&(book.category==="rs_book"||book.category==="search_book")) {
     for (let i = 0; i < this.favorite_books.length; i++) {
      if (this.favorite_books[i].isbn === book.isbn) {
       delete_number = i;
      }
    ...
    else if(book.category==="favorite_book"){
     for (let i = 0; i < this.favorite_books.length; i++) {
      if (this.favorite_books[i].isbn === book.isbn) {
       delete_number = i;
      }
